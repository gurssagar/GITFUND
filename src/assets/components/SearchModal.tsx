    ...
    <div className="z-50 fixed inset-0  bg-opacity-50 flex items-start justify-center pt-[10vh]">
      <div className="bg-white dark:bg-black rounded-xl border border-gray-700 dark:border-[#1a1a1c] w-[50vw] max-h-[70vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="relative p-2">
          <span className="absolute inset-y-0 left-5 text-black dark:text-white flex items-center pointer-events-none">
            <svg
          ...
          />
          <button
            className="absolute top-1/2 right-3 transform -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-[#1a1a1c] cursor-pointer text-gray-400"
            ...
            filteredRepos.map((repo: any) => {
              ...
              const stars =
                typeof repo.stars === "number"
                  ? repo.stars
                  : repo.stars
                    ? parseInt(repo.stars, 10)
                    : 0;
              const forks =
                typeof repo.forks === "number"
                  ? repo.forks
                  : repo.forks
                    ? parseInt(repo.forks, 10)
                    : 0;
              const contributors =
                repo.contributors && Array.isArray(repo.contributors.collabs)
              ...
              return (
                ...
                >
                  <div
                    ...
                    className="hover:bg-gray-100 hover:dark:bg-custom-gray-100 block p-3 rounded-md  transition-colors duration-150"
                  >
                    {" "}
                    {/* Use closeSearchModal from context */}
                    <div className="flex justify-between items-start">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-3 my-1 text-xs dark:text-custom-gray text-gray-500">
                          ...
                          <div className="flex items-center">
                            ...
                            >
                              <path
                                fill="currentColor"
                                d="M22 5a3 3 0 1 0-4 2.816V11H6V7.816a3 3 0 1 0-2 0V11a2 2 0 0 0 2 2h5v4.184a3 3 0 1 0 2 0V13h5a2 2 0 0 0 2-2V7.816A2.99 2.99 0 0 0 22 5"
                              />
                            </svg>
                            <span>{forks}</span>
                          </div>
                      ...
                      {repo.image_url && (
                        <Image
                          ...
                          width={64}
                          height={64}
                          className="rounded object-cover ml-4 flex-shrink-0"
                        />
                      )}
                    </div>
                    {repo.languages &&
                      Object.keys(repo.languages).length > 0 && (
                        <div className="">
                          {Object.keys(repo.languages)
                            ...
                            .map(
                              (
                                language: string, // Show max 5 languages
                              ) => (
                                <span
                                  key={language}
                                  className="inline-block bg-gray-700 dark:bg-[#2a2a2a] text-[11px] text-gray-300 dark:text-gray-400 rounded-full px-2 py-0.5 mr-1.5 mb-1"
                                >
                                  {language}
                                </span>
                              ),
