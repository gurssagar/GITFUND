...
// Update the fetchAssignments function
try {
  const response = await fetch(`/api/requestIssueBasedonProj?${searchParams.toString()}`);
  const data: ApiResponse = await response.json();
  setAssignments(data.result || []);
}

// Update the addContributor function parameters
const addContributor = async (assignment: Assignment) => {
  // Use a unique identifier for loading state, e.g., contributor + issue
  const loadingKey = `${assignment.Contributor_id}-${assignment.issue}`;
  setIsAdding(loadingKey);
  try {
