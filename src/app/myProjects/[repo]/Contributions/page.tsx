    ...
    const fetchBalance = async () => {
      ...
      const handleWithdraw = async (rewardAmount: string): Promise<boolean> => { // Ensure return type is Promise<boolean>
          ...
          try {
              ...
              console.log(`Contract Balance: ${ethers.formatEther(currentContractBalanceWei)} ETH`);
              console.log(`Required Amount:  ${rewardAmount} ETH`);

              if (currentContractBalanceWei < amountInWei) {
                  alert(`Withdrawal failed: Insufficient funds in the contract. Balance: ${ethers.formatEther(currentContractBalanceWei)} ETH, Required: ${rewardAmount} ETH`);
                  console.error(`Insufficient contract balance. Has ${currentContractBalanceWei.toString()} wei, needs ${amountInWei.toString()} wei.`);
                  return false;
              }
              // *** End Balance Check ***

              console.log(`Attempting to withdraw ${rewardAmount} ETH (${amountInWei.toString()} wei) to recipient: ${account}`);
              ...
              alert(`Successfully withdrawn ${rewardAmount} ETH to ${account}`);
