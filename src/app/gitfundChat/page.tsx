    ...
    useEffect(() => {
        ...
        newSocket.on('auth_error', (error) => {
            if (error === 'User already connected') {
                setErrorMessage('You are already connected from another tab or device. Please close other sessions and try again.');
            } else {
        ...
        newSocket.on('reconnect_failed', () => {
            setErrorMessage('Failed to reconnect to the chat server. Please refresh the page.');
        });
                            ...
                            {connectionStatus !== 'connected' && (
                                <div className={`text-center p-4 rounded-lg mb-4 ${
                                    ...
                                    {connectionStatus === 'disconnected' && (
                                        <Button 
                                            onClick={() => onUsernameSelection((session?.user as any)?.username)}
                                            className="mt-2"
                                        >
