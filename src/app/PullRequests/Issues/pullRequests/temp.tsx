...
export default function PullRequestsPage() {
  ...
  console.log("Octokit:", session); // Add this line to check the Octokit instance in the console
  ...
  const getLinkedPRs = useCallback(async (owner: any, repo: string, issueNumber: string) => {
    try {
      ...
      try {
        ...
        .filter(item  => item.event == "cross-referenced" );
        

        console.log(linkedPRs,"linkedPRs");
  ...
  const fetchIssuesAndPRs = useCallback(async () => {
    try {
      ...
      const ContriResponse = await fetch("/api/fetchPullRequests", {
        method: "GET",
      });
      const ContriData = await ContriResponse.json();
      console.log(ContriData.projects, "issuesData");
      setActivePrs(ContriData.projects);
      
      const uniqueContributors = new Set<string>(
        ContriData.projects.map(project => project.Contributor_id)
      );
      console.log("Unique contributors:", [...uniqueContributors]);
      setContributors([...uniqueContributors]);
      

      const issuesResponse=await fetch(`api/fetchIssuesByUser?publisher=${session?.user?.username}`,{
        method:"GET",
      })

      const issuesData=await issuesResponse.json();
      console.log(issuesData.projects,"issuesDatasss");
      
      setIssues(issuesData.projects);
      console.log("Issues:", ContriData.projects);
      ...
      for (const issue of issuesData.projects) {
        try {
          ...
          let owner = ((session.user as any)?.username);
